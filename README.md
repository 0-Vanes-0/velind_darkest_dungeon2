...
"Исправленный" перевод Darkest Dungeon 2 для версии 2.04.80648
...
2. Последний по дате релиз будет в самом верху (например, текущая версия: v0.31). Промотайте ниже и найдите .zip архив (не "Source code", а "Velind_DD2_remastered_<версия>.zip").
...
- Я скачал репозиторий, редактирую перевод (возможно, меня зовут Миша) и хочу использовать авто-патч для игры. Что надо сделать?
  - Первый шаг: В файле `DD2_filepath_to_patch.txt` прописан путь до игры `C:\Program Files (x86)\Steam\steamapps\common\Darkest Dungeon® II`. Если игра расположена в другом месте - нужно поменять в файле.
  - Второй шаг: В Steam ПКМ по игре -> Свойства -> Параметры запуска, вставить туда этот текст: "путь до patch.bat" %command%
  - Путь нужно указать до patch.bat, например как у меня: "D:\Projects\velind_darkest_dungeon2\patch.bat" %command%
